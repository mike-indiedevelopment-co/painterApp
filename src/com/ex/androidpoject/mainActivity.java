...
public class mainActivity extends Activity implements View.OnClickListener {
	...
	private void implementation() {
		...
		// int brushSize = preferences.getInt("BrushSize", 10); you do not need
		// this step
		// int eraserSize = preferences.getInt("EraserSize", 40); you do not
		// need this step
		// int defaultColor = preferences.getInt("DefaultColor", Color.RED); you
		// do not need this step

		// update .setBrushSize, .setEraserSize, and .setColor to pull directly
		// from the preferences
		// and if the value does not exist yet, set the default. we need to
		// change the beginning capital letters
		// to reflect what we are using in the settings. so BrushSize will be
		// brushSize, EraserSize will be eraserSize
		// and DefaultColor will be defaultColor
		((DrawingView) findViewById(R.id.tableDraw)).setBrushSize(preferences.getInt("brushSize", 10)); 	 	// <<< set default brushSize from DrawingView
		((DrawingView) findViewById(R.id.tableDraw)).setEraserSize(preferences.getInt("eraserSize", 40));	 	// <<< set default eraserSize from DrawingView
		((DrawingView) findViewById(R.id.tableDraw)).setColor(preferences.getInt("defaultColor", Color.RED));	// <<< set default color from DrawingView 
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		{
			/*
			 * we do not need this information, we will set the new values
			 * directly from shared prefs and supply a default value if needed.
			 * 
			 * one thing to note here, you are using prefsEd.putInt("BrushSize",
			 * brushSize); and in the settings we are setting "brushSize" so
			 * this will not reference the same values. same goes with the
			 * prefsEd.putInt("EraserSize", eraserSize); should be "eraserSize"
			 * and prefsEd.putInt("DefaultColor", defaultColor); should be
			 * "defaultColor"
			 * 
			 * int brushSize = data.getIntExtra("BrushSize", 1); int eraserSize
			 * = data.getIntExtra("EraserSize", 1); int defaultColor =
			 * data.getIntExtra("DefaultColor", 1);
			 * 
			...
			/** feel free to change the default values, I just went with what was
			  * in the above code you were using this should "hopefully" update the BrushSize, EraserSize, and
			  * Color once we have returned from the settings activity */
			SharedPreferences prefs = PreferenceManager
					.getDefaultSharedPreferences(this);
			((DrawingView) findViewById(R.id.tableDraw)).setBrushSize(prefs
					.getInt("brushSize", 1));
			((DrawingView) findViewById(R.id.tableDraw)).setEraserSize(prefs
					.getInt("eraserSize", 1));
			// you could call changeColor(prefs.getInt("defaultColor",
			((DrawingView) findViewById(R.id.tableDraw)).setColor(prefs.getInt("defaultColor", 1));
			return;
		}
		if (requestCode == 9 && resultCode == RESULT_OK) 								// <<< ColorActivity (color)
		{
			SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
			// int color = data.getIntExtra("Color", 1); we no longer need this
			// if(color != -1)
			/** lets set this directly from shared prefs */
			changeColor(prefs.getInt("defaultColor", 1));
			return;
		}
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
			ContextMenuInfo menuInfo) { 												// <<<< Long Press MENU
	...
	public boolean onContextItemSelected(MenuItem item) { 								// <<<< Implement the Menu categories
		if (item.getItemId() == R.id.itemNew) { 										// <<< NEW
			...
			imp.setItems(content, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					// Toast.makeText(getApplicationContext(), content[which],
			...
			ask.setButton(DialogInterface.BUTTON_POSITIVE, "Ok",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							...
							if (url != null) {
								Toast toastSave = Toast.makeText(
										getApplicationContext(),
										"Picture Saved to Gallery",
										Toast.LENGTH_SHORT);
								toastSave.setGravity(Gravity.CENTER
										| Gravity.CENTER, 0, 0);
								toastSave.show();
							} else {
			...
			startActivityForResult(
					new Intent(mainActivity.this, Settings.class), 7);
